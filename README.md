...
- [Ren'Py：从入门到入坟](#renpy从入门到入坟)
  ...
  - [你好，Ren'Py。](#你好renpy)
    ...
    - [竟然还不会配置开发环境吗，杂鱼~~♥♥](#竟然还不会配置开发环境吗杂鱼)
      - [安装SDK](#安装sdk)
      - [安装IDE](#安装ide)
      - [配置Ren'Py启动器](#配置renpy启动器)
      - [基本操作](#基本操作)
    - [学前准备](#学前准备)
      - [我不学Python了，JOJO！](#我不学python了jojo)
...
同时我也是视觉小说一类游戏的忠实爱好者，很庆幸PyTom大佬能够用Python开发出如此一个极具魅力的游戏引擎，这让更多游戏开发者能够写出高效的代码并且专注于游戏设计。我相信，这样一来将会有更多优秀的作品问世。

在学习并使用了Ren'Py一段时间以后，我就发现一个惨痛的事实————目前Ren'Py的国内社区还并不完善，远远无法和其他游戏引擎相比。我相信各位都有这种经历：经常发现自己看不懂文档、有时候发现问题把文档亦或是论坛翻遍也没得出自己想要的结果，然后疲于在茫茫网络中苦苦寻找。我相信这对于绝大多数开发者来说可以是致命的。所以，一个想法不由而生：制作一份更加详细、易懂、系统的教程来帮助更多初学者入门Ren'Py，最终能够自己独立完成一款Galgame的程序部分；帮助更多Ren'Py开发者写出更加严谨、细致、高效的代码。而且这对于我来说也是一个不小的挑战，在编写这份教程的同时，我也在与你们一起学习。

笔者并非专业游戏开发者，犯错在所难免。但是欢迎提出指正，定会裨补缺漏，为Ren'Py社区尽一份绵薄之力。同时希望大家能够相互促进、共同学习，一起将Ren'Py社区建设得更加完善！————笔者
...
在游戏制作中，我们常常需要加入图片、音频、视频等元素。

我们可以在Ren'Py中加载这些文件，但是Ren'Py支持渲染的图像或播放的音频的文件格式是有限制的，支持的文件格式具体如下：
...
如果你是零基础，那么推荐你去一些视频网站（如 *Bilibili*）找Python相关内容学习，。

对于Python知识的要求：掌握Python的基本数据类型、流程控制语句、常用的内建函数以及基本的面向对象编程思想（面向对象解决一切，好耶）。
...
1. 所有脚本文件中定义的变量都能够被跨文件访问。例如你在一个脚本文件中定义了变量 *var* ，那么在任意一个脚本文件中都能够访问该变量 *var*。
2. 所有脚本文件中的变量均不能重名。
...
此处的`[]`用于格式化字符串，可在[这部分内容](#内插字符)中查看
