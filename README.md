...
- [Ren'Py：从入门到入坟](#renpy从入门到入坟)
  ...
  - [你好，Ren'Py。](#你好renpy)
    ...
    - [竟然还不会配置开发环境吗？](#竟然还不会配置开发环境吗)
      - [安装SDK](#安装sdk)
      - [安装IDE](#安装ide)
      - [配置Ren'Py启动器](#配置renpy启动器)
      - [基本操作](#基本操作)
    - [学前准备](#学前准备)
      - [我不学Python了，Ren'Py！](#我不学python了renpy)
  ...
  - [Ren'Py语法](#renpy语法)
    ...
    - [脚本标签（label）](#脚本标签label)
      ...
      - [return语句](#return语句)
        - [携带返回值](#携带返回值)
        - [标签穿透————流程，你不要停下来啊！](#标签穿透流程你不要停下来啊)
      - [特殊标签名](#特殊标签名)
      - [全局（global）与局部（local）脚本标签（不常用）](#全局global与局部local脚本标签不常用)
    - [对话呈现](#对话呈现)
      - [say语句](#say语句)
      - [特殊文本样式](#特殊文本样式)
        - [转义符](#转义符)
...
同时我也是视觉小说一类游戏的忠实爱好者，很庆幸PyTom大佬能够用Python开发出如此一个极具魅力的游戏引擎。这不仅让更多游戏开发者能够专注于游戏设计，也将促使更多优秀的作品问世。

在学习并使用了Ren'Py一段时间以后，我就发现一个惨痛的事实————目前Ren'Py的国内社区还并不完善，远远无法和其他游戏引擎相比。我相信各位都有这种经历：经常发现自己看不懂文档、有时候发现问题把文档亦或是论坛翻遍也没得出自己想要的结果，然后疲于在茫茫网络中苦苦寻找。我相信这对于绝大多数开发者来说可以是致命的。所以，一个想法不由而生：制作一份更加详细、易懂、系统的教程来帮助更多初学者入门Ren'Py，最终能够自己独立完成一款视觉小说的程序部分；帮助更多Ren'Py开发者写出更加严谨、细致、高效的代码。同时这对于我来说也是一个不小的挑战，在编写这份教程的同时，我也在与你们一起学习。

笔者并非专业游戏开发者亦非专业软件工程师，犯错在所难免。但是欢迎提出指正，定会裨补缺漏，为Ren'Py社区尽一份绵薄之力。同时希望大家能够相互促进、共同学习，一起将Ren'Py社区建设得更加完善！————ZYKsslm
...
简而言之，Ren'Py就是一个能够帮助我们进行游戏开发的工具，并达到事半功倍的效果。
...
也就是说，我们不必再去从头开始搭建一个GUI游戏界面，并去实现繁琐的基础功能或搭建框架，这些Ren'Py都会提供给我们，我们只需要专注于游戏设计。

>跨平台性：前面提到过跨平台性，跨平台意指只需写一次代码，程序就能在多个不同的环境（操作系统）中运行，这将大大节省我们的时间，提高开发效率。基于Python的特性，目前Ren'Py支持的平台有Windows、Linux、Mac OS X、Android和iOS以及不太常用的Web前端（浏览器）。
...
我们可以在Ren'Py中加载这些资源文件，但是Ren'Py支持渲染的图像或播放的音频的文件格式是有限制的，支持的文件格式具体如下（未经完全测试）：
...
SDK即为Software Development Kit（软件开发工具包），用于给开发者提供开发工具，这就是Ren'Py本体了（包括一个Ren'Py Launcher）。安装完SDK我们就可以使用Ren'Py来运行我们编写的脚本代码了。
...
安装完成后，我们打开vscode，打开扩展商店搜索`renpy`并下载Ren'Py扩展插件（最新官方版插件作者名称为`Ren'Py`）。

![addition_renpy](resources/images/addition_renpy.jpg)
...
由于Ren'Py是基于Python的，所以有很多概念和语法都和Python相似甚至是一样。你甚至可以几乎完全使用Python来使用Ren'Py引擎。

**因此我们强烈建议所有Ren'Py使用者都掌握Python基础**。

如果你已经熟练掌握Python，那么相信学习Ren'Py对你来说并不是什么难事，甚至只是相当于学习一个第三方库而已。所以，先熟悉Python将会使你的Ren'Py学习之路更加平坦。本教程会可能会涉及一些Python内容。
...
在游戏中我们可以使用`config.gamedir`来获取当前游戏目录（即使游戏打包后依然可行）：
...
在我们每次编写完代码重启Ren'Py时，Ren'Py会自动生成或更新rpyc文件，再次运行时会直接运行rpyc文件而省去编译的过程。

所以在没有rpy文件而有rpyc文件的情况下，游戏依然能够运行其功能。但是除了发布版本之外最好不要删除rpy文件。
...
同时，Ren'Py中还有一些预留名同样不能作为变量名复用，否则将引发错误。
...
和Python一样，在Ren'Py中我们使用符号`#`放在一条语句的**开头**来表示一条注释。注释可以放在任何位置，但不能插在一条代码中间，我们一般置于代码的上一行或末尾。
...
对于define语句，我们使用形如`define var = value`的方式来給一个变量赋值。
...
对于default语句，我们使用形如`default var = value`的方式来給一个变量赋值。

```renpy
default point = 10
```

**default语句可以放在脚本文件的任何位置，但一般建议放在脚本文件的开头（顶级位置）。**
...
1. 定义临时变量
2. 修改已经定义过的变量（如使用`default`语句定义的变量）
3. 调用函数或方法
...
Ren'Py中所有的游戏流程代码都会分别写进数个脚本标签中。脚本标签类似于Python中的函数，用于跳转或调用。但不同的是，在Ren'Py中，写代码时脚本标签可以在定义前调用（定义语句总会优先运行）。

我们使用`label`关键字来定义一个脚本标签，然后是标签名，后面可紧跟一对小括号以添加参数，若没有参数则括号可以省略，最后别忘了加冒号并换行缩进。参数的写法、作用域等与Python函数完全相同。
...
label start:
  ...
  $ renpy.call("sample_label", content="这是Python等效语句")  # Python等效语句
...
label start:
  ...
  jump sample_label
  $ renpy.jump("sample_label")  # Python等效语句
...
2. 在一个全局脚本标签中可以直接访问与之关联的局部脚本标签，而想要访问其它全局脚本标签关联的局部脚本标签则需要使用 *全局脚本标签名.局部脚本标签名* 的形式来访问。
3. 在一个局部脚本标签中可以直接访问与之关联同一个全局脚本标签的另一局部脚本标签，而想要访问其它全局脚本标签关联的局部脚本标签依然需要使用 *全局脚本标签名.局部脚本标签名* 的形式来访问。
...
label .local_label:
  ...
  "这是局部脚本标签.local_label_1，关联global_label全局脚本标签"

  return
```

在上面的代码中，有 *start* 和 *globel_label* 全局脚本标签，其分别关联了 *.local_label* 局部脚本标签。
...
4. 由一个角色（Character）对象和一个字符串组成，角色对象控制发言展现的形式，字符串则是显示的内容。这种形式也是最为常用的。

>有些say语句会在原有基础上加上一个 *with* 从句，`with`后跟上一个转场（transition）来达到过渡效果。
...
由三引号引起来的文本，Ren’Py会根据内容中的空行自动分段。分段后的每一段内容，都会创建自身的say语句。同时，由于nvl角色默认共用一个界面，所以说完话后记得使用`nvl clear`清空nvl界面。关于nvl角色的更多用法，请查看后文。
...
```renpy
image bg night = "resources/images/bg_night.jpg"
```
...
image语句还能把一个**可视组件**或一个**表示颜色的十六进制数的字符串**（其实也算是可视组件）定义为图像：

```renpy
image bg night = "resources/images/bg girl.jpg"
image black = "#000000"
