...
define语句用于让初始化阶段给一个变量赋值，**用define定义的变量会被当作一个常量，不应修改**。
...
3. 可能有些say语句会在原有基础上加上一个 *with* 从句，`with`后跟上一个转场（transition）来达到过渡效果。
...
我们先讨论前三种形式。看到上面的代码，会先输出`这是一句旁白`，而后再以“我”的发言状态输出`这是我的内心活动`，对话框的左上角有一个发言人的名字`我`，最后再输出`这是真的吗？！`同时伴有窗口抖动效果，*vpunch* 是一个内置转场，在with语句中可以达到让窗口抖动的效果。同时，Ren'Py还内置了很多如*vpunch* 的转场，可以在[官方文档](https://www.renpy.cn/doc/transitions.html#pre-defined-transitions)中查看。
...
![character_sample](images/character_sample.jpg)
