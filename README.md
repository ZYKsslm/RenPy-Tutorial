...
在游戏中我们可以使用`config.game_dir`来获取当前游戏目录（即使游戏打包后依然可行）：
...
1. 所有脚本文件中定义的变量都能够被“跨文件”访问。例如你在一个脚本文件中定义了变量 *var* ，那么在任意一个脚本文件中都能够访问该变量 *var*。
2. 所有脚本文件中的变量均不能重名。
...
所以在没有rpy文件而有rpyc文件的情况下，游戏依然能够运行其功能。但是最好不要删除rpy文件，因为可能会发生意料之外的错误。
...
通常来说，`default`语句的Python等效语句为Python赋值语句。
